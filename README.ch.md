...
- **仅复习 / 自测**：界面有独立的「仅复习」区块，**不必**先「添加笔记」。可从知识库选择已有 `notes/*.md`，或粘贴任意 Markdown，由 LLM 根据正文**抽一道**复习问答题；可查看原文对答案、再**换一题**。与识别 pipeline 共用 API Key；无 Key 且开启 `TOMATO_USE_STUB=1` 时为占位题。
...
5. 在**仅复习**（位于「添加笔记」下方）：从知识库下拉选一篇，或粘贴内容后点「使用粘贴内容」，再**抽一道题** / **换一题**；**查看原笔记**可展开对照。若刚跑完一次识别，可在结果区用**在复习区打开本结果**把当前 Markdown 填进复习区。
...
- `GET /api/kb`：知识库仪表信息（路径、笔记数、维护状态等）。
